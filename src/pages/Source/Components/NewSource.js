        ...
        <div>
            ...
            this.state.view ? (
              <div className="main-modal">
                <Box sx={{ height: '200px', width: '300px', borderRadius: '4px', padding: '16px' }}>
                  <Grid style={{ display: 'flex', marginBottom: '7px', margin:'0px 8px', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography style={{ color: '#000', fontWeight: '500', fontSize: '24px', }}> View Source</Typography>
                  ...
                  <Grid style={{ display: 'flex', gap: '15px', justifyContent: 'center' }}>
                    <FormControl style={{ width: '250px' }} variant="standard" sx={{ m: 1, minWidth: 120 }}>
                      <InputLabel id="demo-simple-select-helper-label">View Source</InputLabel>
                      <Select
                        labelId="demo-simple-select-helper-label"
                        id="demo-simple-select-helper"
                        label="Source"
                  ...
                  <Button onClick={() => this.setState({ view: !this.state.view })} variant="contained" style={{ float: 'center', marginTop: '15px', marginLeft: '75px', padding: '5px 30px', backgroundColor: '#AA3F1D' }}>Okay</Button>
                </Box>
              </div>
            ) : null
          }
            ...
            this.state.delet ? (
              <div className="main-modal-delet">
                <Box sx={{ height: '200px', width: '300px', borderRadius: '4px'}}>
                  <Grid style={{ display: 'flex', padding:'0px 10px', boxShadow:'0px 1px 0px 0px grey', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  ...
                  <Grid style={{ display: 'flex',  width: '237px' }}>
                    <Typography style={{ height: '36px', color: '#000', whiteSpace: 'nowrap', marginLeft:'30px', wordSpacing:'-2px' }}>Are You Sure You Want To Delete !</Typography>
                  </Grid>
                  <Grid style={{ display: 'flex', justifyContent: 'space-between', gap: '15px', margin: '33px 40px 0px 37px' }}>
          ...
          }
          {
            this.state.edit ? (
              <div className="main-modal">
                <Box sx={{ height: '200px', width: '300px', borderRadius: '4px', padding: '16px' }}>
                  <Grid style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin:'0px 6px' }}>
                  ...
                  {rows.map((row) => (
                    <StyledTableRow key={row.Category}>
                      ...
                      <StyledTableCell align="right">
                        <EditIcon className='icons' onClick={() => this.setState({ edit: true })} /> <VisibilityIcon onClick={() => this.setState({ view: !this.state.view })}/> <DeleteIcon onClick={() => this.setState({ delet: true })} />
                      </StyledTableCell>
                    </StyledTableRow>
                  ))}
                </TableBody>
